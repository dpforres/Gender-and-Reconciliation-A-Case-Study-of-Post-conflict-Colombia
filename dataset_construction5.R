...
########### Create peace process support variable ##############
lapop_2013 <- lapop_2013 %>%
  mutate(
    peace = na_if(colpropaz1, 98)
  )
...
# Change name of colpaz4an (reintegration of FARC) in 2016 to colpaz4a
colnames(lapop_2016)[colnames(lapop_2016) == "colpaz4an"] <- "colpaz4a"

# Change name of colpaz4bn (reinitegration of BACRIM) in 2016 to colpaz4b
colnames(lapop_2016)[colnames(lapop_2016) == "colpaz4bn"] <- "colpaz4b"
...
common_cols <- c("uniq_id", "year", "q1", "q2y", "ed", "wc1", "wc3", "wc2", 
                 ...
                 "colrecon8", "colrecon18", "centro", "ur", "l1", "colpaz4a", "colpaz4b", "colpropaz5",
                 "colwc5", "colwc6", "colwc7", "colwc8", "colwc9", "coljp5", "colconce1",
                 "colrecon3", "colconce7", "coldis35f", "colrecon12", "colpropaz6", "colpropaz11a", "colpact1",
                 "colpropaz13k", "coljepaa1", "colconce6", "peace")
...
prov_lookup_all %>%
  dplyr::distinct(prov, prov_label) %>%
  dplyr::count(prov) %>%
  dplyr::filter(n > 1)
